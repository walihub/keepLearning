...
class Spider:
    ...
    def get_all_questions(self):
        ...
        questions.sort(key=lambda x: x['stat']['frontend_question_id'])

        for question in questions[:self.max_num]:
            ...
            path_cn = '/leetcode/{}/{}.{}/README.md'.format(number_mapper.get(str(no)), frontend_question_id,
                                                            quote(question_title_en))
            path_en = '/leetcode/{}/{}.{}/README_EN.md'.format(number_mapper.get(str(no)), frontend_question_id,
                                                               quote(question_title_en))
            ...
            if (int(frontend_question_id) + 1) % 100 == 0:
                json_result = {"leetcode": self.result}
                with open('leetcode{}.json'.format(frontend_question_id), 'w', encoding='utf-8') as f:
    ...
    def generate_md_table(self):
        """生成md题目列表"""
        writer = open("problem_list.txt", 'w', encoding='utf-8')
        writer.write("|  题号  |  题解  |  标签  |  难度  | 备注 |\n")
        writer.write("| --- | --- | --- | --- | --- |\n")
        ...
        writer.write("|  #  |  Solution  |  Tags  |  Difficulty  |  Remark |\n")
        writer.write("| --- | --- | --- | --- | --- |\n")
...
if __name__ == '__main__':
    ...
    for file in glob.glob('./leetcode*.json'):
        with open(file, 'r', encoding='utf-8') as f:
            result = json.load(f)
            for item in result['leetcode']:
                no = int(item['frontend_question_id']) // 100

                path = f'../../leetcode/{number_mapper.get(str(no))}/{item["frontend_question_id"]}.{item["title_en"]}'
                path = path.replace(":", " ")
